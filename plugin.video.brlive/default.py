...
addon = xbmcaddon.Addon('plugin.video.Brlive')
addonname = ('[COLOR white][B]Br[/B][CLOR red][B]Live[/B][/COLOR]')
...
CHBase = base64.decodestring('aHR0cDovL2JpdC5seS9CckxpdmUyMDE5')

def CHIndex():
    addon_log("CHIndex")
    getData(CHBase,'')
    msg = "https://pastebin.com/raw/NtvUVZvh"
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.Brlive-%s]: %s" %(addon_version, string))
...
def SKindex():
    addon_log("SKindex")
    getData(ksd,'')
    msg2 = ("https://pastebin.com/raw/NtvUVZvh")				
    msg = msg2 
    line1 = urllib2.urlopen(msg).read()
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(Brlive,Novos Conteúdos Adicionados.,5000,"+icon+")")
        if not url is None:
...
def SearchChannels():
    ...
    progress.create('Brlive Procurando Espere',' ')
...
#            print 'parentalblockedpin',parentalblockedpin
            ...
            if not name in FAV:
                contextMenu.append(('Add to Brlive Favorites','XBMC.RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
...
#                print 'setting as folder in link'
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                contextMenu.append(
                    ('Remove from Brlive Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
                     %(sys.argv[0], urllib.quote_plus(name)))
                     )
                ...
                contextMenu.append(('Add to Brlive Favorites','XBMC.RunPlugin(%s)' %fav_params))
            liz.addContextMenuItems(contextMenu)
...
#                print 'proxy',proxy
            ...
            xbmc.executebuiltin("XBMC.Notification(Brlive,Failed to extract regex. - "+"this"+",4000,"+icon+")")
elif mode==18:
    addon_log("youtubedl")
    try:
        import youtubedl
    except Exception:
        xbmc.executebuiltin("XBMC.Notification(Brlive,Please [COLOR yellow]install Youtube-dl[/COLOR] module ,10000,"")")
    ...
    if not (keyboard.isConfirmed() == False):
        ...
        if newStr==parentalblockedpin:
            ...
            xbmc.executebuiltin("XBMC.Notification(Brlive,Parental Block Disabled,5000,"+icon+")")
        else:
            xbmc.executebuiltin("XBMC.Notification(Brlive,Wrong Pin??,5000,"+icon+")")
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
elif mode==56:
    addon_log("disable lock")
    addon.setSetting('parentalblocked', "true")
    xbmc.executebuiltin("XBMC.Notification(Brlive,Parental block enabled,5000,"+icon+")")
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
