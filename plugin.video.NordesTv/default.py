...
#######################################################################################
####  TITULO DO MENU  #################################################################
title_menu = '[B][COLOR blue]:::[/COLOR][COLOR white]::[/COLOR] [COLOR white]BEM VINDOS[/COLOR] [COLOR white]::[/COLOR][COLOR blue]:::[/COLOR][/B]'
#######################################################################################

#######################################################################################
###  DESCRIÇÃO DO ADDON ###############################################################
### OBS: PARA SALTAR UMA LINHA UTILIZE \n #############################################
url_b64_title_descricao = 'aHR0cHM6Ly9wYXN0ZWJpbi5jb20vcmF3L25wa2V1UzdM'
url_title_descricao = base64.b64decode(url_b64_title_descricao)
#######################################################################################

####  SUPORTE PREMIUM  #################################################################
title_menu2 = '[B][COLOR blue]:::[/COLOR][COLOR white]::[/COLOR] [COLOR white]SUPORTE[/COLOR][/B][COLOR yellow]:[/COLOR] [B][COLOR gold]PREMIUM[/COLOR] [COLOR white]::[/COLOR][COLOR blue]:::[/COLOR][/B]'
#######################################################################################

#######################################################################################
###  DESCRIÇÃO DO SUPORTE PREMIUM #########################################################
### OBS: PARA SALTAR UMA LINHA UTILIZE \n #############################################
url_b64_title_descricao2 = 'aHR0cHM6Ly9wYXN0ZWJpbi5jb20vcmF3L0JqYWNTSHFh'
url_title_descricao2 = base64.b64decode(url_b64_title_descricao2)
#######################################################################################

####  MENSAGEM - BASE 64 ##############################################################
url_b64_mensagem = 'aHR0cHM6Ly9wYXN0ZWJpbi5jb20vcmF3L2tDV1AyVmU1'
...
useragent = 'N0rdesTV/3.2.4'
##############################################################################

#### MENU PREMIUM ################################################################
titulo_PREMIUM = '[B][COLOR blue]Nordes[/COLOR][COLOR white]TV[/COLOR][/B] [COLOR yellow]:[/COLOR] [B][COLOR gold](PREMIUM)[/COLOR][/B]'
thumbnail_PREMIUM = 'https://i.imgur.com/hmsuEsX.jpg'
fanart_PREMIUM = 'https://i.imgur.com/hmsuEsX.jpg'
#### DESCRIÇÃO PREMIUM ###########################################################
url_b64_PREMIUM_descricao = 'aHR0cHM6Ly9wYXN0ZWJpbi5jb20vcmF3L0JqYWNTSHFh'
url_PREMIUM_descricao = base64.b64decode(url_b64_PREMIUM_descricao)

#### DIALOGO PREMIUM - SERVIDOR NENHUM - CLICK ###################################
url_b64_PREMIUM_dialogo = 'aHR0cHM6Ly9wYXN0ZWJpbi5jb20vcmF3L3J5R1dMZW5n'
url_PREMIUM_dialogo = base64.b64decode(url_b64_PREMIUM_dialogo)

#### MENU PREMIUM2 ################################################################
titulo_PREMIUM2 = '[B][COLOR blue]Nordes[/COLOR][COLOR white]TV[/COLOR][/B] [COLOR yellow]:[/COLOR] [B][COLOR gold](PREMIUM2)[/COLOR][/B]'
thumbnail_PREMIUM2 = 'https://i.imgur.com/fkWB47r.jpg'
fanart_PREMIUM2 = 'https://i.imgur.com/fkWB47r.jpg'
...
def addon_log(string):
    if debug == 'true':
       # xbmc.log("[addon.live.SimpleKore Lists-%s]: %s" %(addon_version, string))
        xbmc.log("[NordesTv-%s]: %s" %(addon_version, string))
...
########################################################################################################################
# INICO DOS MODULOS A SER COLOCADOS FORA DO DEFAULT 

#modulo modulos
exec(module_import('https://pastebin.com/raw/fD4b8xej')) 



####################################################################
#### INICIO DO MODULO UPDATE ABAIXO SE QUISER PODE DEIXAR TD EM UM MODOLO SÓ OK!!!!
exec(module_import('https://pastebin.com/raw/LPtvvJNC'))
            ...
            if len(sources) > 1:
                for i in sources:
                    ...
                    if isinstance(i, list):
                        ...
                        if i.has_key('rate'):
                            rate = i['rate']
                        addDir(i['title'].encode('utf-8'),i['url'].encode('utf-8'),1,thumb,fanart,desc,genre,date,credits,year,director,duration,premiered,studio,rate,'source')
    ...
    for other,channel_name,stream_url in match:
        ...
        if 'type' in other:
            ...
            if mode_type == 'yt-dl':
                ...
                addLink2(url[0], channel_name,thumbnail,'','','','','',None,regexs,total)
                continue
            ...
            try:
                ...
                if len(item('link')) >0:
                    ...
                    for i in item('gdrive'):
                        if not i.string == None:
                            if len(i.string) == 33:                                
                                gdrive = 'plugin://plugin.video.gdrive?mode=streamURL&amp;url=https://drive.google.com/open?id='+urllib.quote_plus(codecs.encode(base64.b32encode(base64.b16encode(url)), 'rot_13'))+ i.string
                                #isJsonrpc=gdrive
                        url.append(gdrive)
                    #############################
            ...
            try:
                ...
                if len(url) > 1:
                    ...
                    if isXMLSource:
                        ...
                        addLink2(url[0],name.encode('utf-8', 'ignore'),thumbnail,fanArt,desc,genre,date,year,director,duration,premiered,studio,rate,True,None,regexs,total)
                    #print 'success'
            except:
...
def urlsolver(url):
    ...
    ##genesis_url = 'https://raw.githubusercontent.com/lambda81/lambda-addons/master/plugin.video.genesis/commonresolvers.py'
        genesis_url = 'https://raw.githubusercontent.com/NordesTvHD/OneRepo/master/plugin.video.genesis/commonresolvers.py'
        ...
        if '$$LSPlayOnlyOne$$' in mu_playlist[0]:
            ...
            for i in mu_playlist:
                ...
                if regexdialog in i:
                    ...
                    d_name=i.split(regexdialog)[1].split('®exs')[0]
                    names.append(d_name)                                       
                    #mu_playlist[iloop]=i.split('$$lsname=')[0]+('®exs'+i.split('®exs')[1] if '®exs' in i else '')                    
                    mu_playlist[iloop]=i.split(regexdialog)[0]+('®exs'+i.split('®exs')[1] if '®exs' in i else '')                    
                else:
...
#            print 'parentalblockedpin',parentalblockedpin
            ...
            if showcontext == 'source':
                ...
                contextMenu.append(('Remove from NordesTv Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
            ...
            if showcontext == '!!update':
                ...
                contextMenu.append(('[COLOR aquamarine]!!update[/COLOR]','XBMC.RunPlugin(%s)' %fav_params2))
            if not name in FAV:
                fav_params = (
                ...
                contextMenu.append(('Adicionar [COLOR aquamarine]NordesTv[/COLOR]','XBMC.RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
...
def addLink2(url,name,iconimage,fanart,description,genre,date,year,director,duration,premiered,studio,rate,showcontext,playlist,regexs,total,setCookie="",allinfo={}):
...
#                print 'setting as folder in link'
        ...
        if playlist:
            if addon.getSetting('add_playlist') == "false" and '$$LSPlayOnlyOne$$' not in playlist[0] :
                ...
                name = name + '[COLOR firebrick] (' + str(len(playlist)) + ' itens)[/COLOR]'
                play_list = True
        else:
            u += "url="+urllib.quote_plus(url)+"&mode="+mode
        if regexs:
        ...
        if allinfo==None or len(allinfo) <1:
            liz.setInfo(type="Video", infoLabels={ "Title": name, "Plot": description, "Genre": genre, "dateadded": date, "Year": year, "Director": director, "Duration": duration, "Premiered": premiered, "Studio": studio, "Mpaa":rate })
        else:
            liz.setInfo(type="Video", infoLabels=allinfo)
        liz.setProperty("Fanart_Image", fanart)
...
#                print 'setting as folder in link'
        ...
        if playlist:
            if addon.getSetting('add_playlist') == "false" and '$$LSPlayOnlyOne$$' not in playlist[0] :
                ...
                u += "mode=13&name=%s&playlist=%s" %(urllib.quote_plus(name), urllib.quote_plus(str(playlist).replace(',','||')))
                name = name + '[COLOR blue] (' + str(len(playlist)) + ' itens)[/COLOR]'
                play_list = True
        else:
            u += "url="+urllib.quote_plus(url)+"&mode="+mode
        if regexs:
...
#    xbmcplugin.endOfDirectory(int(sys.argv[1]))
    ...
    addon_log("puxarconf") 
    xbmcgui.Dialog().ok('[B][COLOR white]CONFIGURAÇÕES![/COLOR][/B]','[B][COLOR yellow]|[/COLOR][/B] [B]ATIVAR E DESATIVAR AS MENSAGENS [/B][B][COLOR yellow]|[/COLOR] CONFIGURAR PREMIUM[/B] [B][COLOR yellow]|[/COLOR][/B]')   
    xbmcaddon.Addon().openSettings()
    xbmcgui.Dialog().ok('[B][COLOR red]AVISO IMPORTANTE![/COLOR][/B]','[B][COLOR yellow]|[/COLOR][/B][COLOR white] POR FAVOR SAIR DO ADD-ON E ENTRE NOVAMENTE,[B] [COLOR yellow]|[/COLOR][/B] PARA ATUALIZAR AS CONFIGURAÇÕES![B] [COLOR yellow]|[/COLOR][/B]')
    xbmc.executebuiltin("XBMC.Container.Refresh(%s?mode=None, replace)"%sys.argv[0])
    xbmcplugin.endOfDirectory(sys.exit(0))
	
elif mode==42:
    addon_log("infoPREMIUM")    
    ##xbmcaddon.Addon().openSettings()
    #xbmcgui.Dialog().ok('[COLOR gold][B](PREMIUM) [/B][/COLOR][COLOR aquamarine][B]AREA ASSINANTES[/B][/COLOR]', dialog4)
    xbmcgui.Dialog().ok(titulo_PREMIUM, descricao(url_PREMIUM_dialogo))
    ##xbmc.executebuiltin("XBMC.Container.Refresh")
