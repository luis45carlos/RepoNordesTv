...
iconpath = xbmc.translatePath(os.path.join('special://home/addons/script.keltecmpmaintenance','icon.png'))
...
if count > maxpackage_zips or int(total_sizetext) > filesize:
	choice2 = xbmcgui.Dialog().yesno("[COLOR=red]Autocleaner[/COLOR]", 'A pasta de pacotes e [COLOR red]' + str(total_sizetext) +' MB [/COLOR] - [COLOR red]' + str(count) + '[/COLOR] zip files', 'A pasta pode ser limpa sem problemas para economizar espaco...', 'Voce quer limpa-lo agora?', yeslabel='Sim',nolabel='Nao')
	if choice2 == 1:
...
if int(total_sizetext2) > filesize_thumb:
	choice2 = xbmcgui.Dialog().yesno("[COLOR=red]Autocleaner[/COLOR]", 'A pasta de imagens e [COLOR red]' + str(total_sizetext2) + ' MB   [/COLOR]', 'A pasta pode ser limpa sem problemas para economizar espaco...', 'Voce quer limpa-lo agora?', yeslabel='Sim',nolabel='Nao')
	if choice2 == 1:
		control.deleteThumbnails()

xbmc.executebuiltin('XBMC.Notification(%s, %s, %s, %s)' % ('KelTecMP-Status da Limpeza',  'Packages: '+ str(total_sizetext) +  ' MB'  ' - Images: ' + str(total_sizetext2) + ' MB' , '5000', iconpath))